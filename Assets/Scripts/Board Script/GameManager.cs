    ...
    private void SyncGameState()
    {
        ...
        var stateProps = new ExitGames.Client.Photon.Hashtable()
        {
            {"GameState", (int)currentState},
            {"BossTurn", isBossTurn},
            {"CurrentTurn", RoundManager.Instance.Turn},
            {"CurrentPlayerID", PlayerManager.Instance.CurrentPlayerID}
        };
        ...
        private void ChangeState(GameState newState)
        {
            ...
            {
                photonView.RPC("RPCChangeState", RpcTarget.All, (int)newState);
                SyncGameState(); // Sync to room properties as well
            }
            else
            {
                Debug.LogWarning("Non-master client attempted to change game state.");
            }
        }
        public override void OnPlayerEnteredRoom(Photon.Realtime.Player newPlayer)
        {
            ...
            if (PhotonNetwork.IsMasterClient)
            {
                ...
                photonView.RPC("RPCSyncFullState", newPlayer, 
                    (int)currentState,
                    isBossTurn,
                    RoundManager.Instance.Turn,
                    PlayerManager.Instance.CurrentPlayerID);
            }
        }
        [PunRPC]
        private void RPCChangeState(int newStateValue)
        {
            currentState = (GameState)newStateValue;
            Debug.Log($"[GameManager] State changed to {currentState}");
            OnStateChanged(currentState);
            OnGameStateChanged?.Invoke(currentState);
        }
        [PunRPC]
        private void RPCSyncFullState(int state, bool bossTurn, int currentTurn, int currentPlayerId)
        {
            currentState = (GameState)state;
            isBossTurn = bossTurn;
            RoundManager.Instance.Turn = currentTurn;
            PlayerManager.Instance.CurrentPlayerID = currentPlayerId;
            
            OnStateChanged(currentState);
            UIManager.Instance.UpdateCurrentPlayerTurn(currentTurn);
        }
        public override void OnRoomPropertiesUpdate(ExitGames.Client.Photon.Hashtable propertiesThatChanged)
        {
            if (propertiesThatChanged.ContainsKey("GameState"))
            {
                GameState newState = (GameState)propertiesThatChanged["GameState"];
        ...
        {
            if (isBossTurn) return PhotonNetwork.IsMasterClient;
            
            Player currentPlayer = PlayerManager.Instance.GetCurrentPlayer();
            return currentPlayer != null && 
                currentPlayer.photonView.IsMine;
        }
